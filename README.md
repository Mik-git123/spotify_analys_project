...
```sql
select sum(comments) as tot_comments
from spotify
where licensed = 'True';
```
...
```sql
select artist,count(*) as tot_tracks
from spotify
group by artist
order by 2;
```
...
```sql
select album,avg(danceability) as avg_dancea
from spotify
group by album
order by 2 desc;
```
...
(select 
	track,
	--most_played_on,
	COALESCE(sum(CASE WHEN most_played_on = 'Youtube' THEN stream END),0) as stream_yt,
	COALESCE(sum(CASE WHEN most_played_on = 'Spotify' THEN stream END),0) as stream_sp
from spotify
group by 1
) as max_stream_sp
where
